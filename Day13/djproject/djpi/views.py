...
def he(request):
	print("Hello World")
	return HttpResponse("Welcome to Django internship")

def hlp(req,n):
	return HttpResponse("Hi {} Welcome to DjangoInternship".format(n))

def tbl(request,v):
	y = []
	for i in range(1,11):
		u = "{} x {:02} = {:02}".format(v,i,v*i)+"<br/>"
		y.append(u)
	##print(y)
...
def stdnt(request,prc,name,age):
	t = "<h5 style='background-color:peach:color:black;font-size:20px;'>Hi Welcome To Django</h5>".format(name)
